...
public class TaskManager implements TaskOperationProtocol {
	...
	private static final String[] checkTempDirs(final String[] tempDirs) throws Exception {
		...
		final int treshold = GlobalConfiguration.getInteger(ConfigConstants.TASK_MANAGER_TMP_DIR_THRESHOLD_KEY, 
				ConfigConstants.DEFAULT_TASK_MANAGER_TMP_DIR_TRESHOLD);
		...
		for (int i = 0; i < tempDirs.length; ++i) {
			...
			if((f.getUsableSpace() / (1024 * 1024)) < treshold)
			{
				LOG.warn(String.format("Temporary file directory '%s' was removed from the list "
						+ "because it has less storage than the defined treshold (%d MB) left.", f.getAbsolutePath(), treshold));
